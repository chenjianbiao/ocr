...
tf.app.flags.DEFINE_integer('charset_size', 3755, "Choose the first `charset_size` characters only.")
# tf.app.flags.DEFINE_integer('image_size', 100, "Needs to provide same value as in training.")
...
tf.app.flags.DEFINE_string('train_data_dir', 'c:\\dataset0506\\train', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', 'c:\\dataset0506\\test', 'the test dataset dir')
...
class DataIterator:
    ...
    def input_pipline(self, batch_size, num_epochs=None, aug=False):
        ...
        #文件内容转换为tensor
        images = tf.convert_to_tensor(tf.to_float(tf.image.decode_png(images_content, channels=1)), dtype=tf.float32)

        if aug:
            images = self.data_augmentation(images)
        new_size = tf.constant([FLAGS.image_size, FLAGS.image_size], dtype=tf.int32)
        images = tf.image.resize_images(images, new_size)
        image_batch, label_batch = tf.train.shuffle_batch([images, labels],min_after_dequeue=batch_size, batch_size=batch_size, capacity=50000)
...
def train():
    ...
    train_feeder = DataIterator(data_dir='c:\\dataset0506\\train\\')
    test_feeder = DataIterator(data_dir='c:\\dataset0506\\test\\')
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=False)) as sess:
        train_images, train_labels = train_feeder.input_pipline(batch_size=FLAGS.batch_size, num_epochs=1, aug=True)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                print("i=%d" % i)
                # print(sess.run(tf.cast(graph['global_step'],dtype=tf.int32)))
...
def validation():
    ...
    test_feeder = DataIterator(data_dir='c:/dataset0506/test/')
    final_predict_val = []
    final_predict_index = []
    groundtruth = []
